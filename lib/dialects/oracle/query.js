...
QueryCompiler_Oracle.prototype.insert = function () {
  ...
  if (returning) {
    ...
    sql.returningSql = 'select ' + this.formatter.columnize(returning) + ' from ' + self.tableName + ' where ROWID in (' + sql.outParams.map(function (v, i) {
      return ':' + (i + 1);
...
function surroundQueryWithLimitAndOffset(self, query, limit, offset) {
  ...
  return 'select * from ' + '(select row_.*, ROWNUM rownum_ from (' + query + ') row_ ' + 'where rownum <= ' + self.formatter.parameter(endRow) + ') ' + 'where rownum_ > ' + self.formatter.parameter(offset);
}
...
QueryCompiler_Oracle.prototype.first = QueryCompiler_Oracle.prototype.select = function () {
  var self = this;
